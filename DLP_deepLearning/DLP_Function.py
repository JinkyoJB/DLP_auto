...
#함수1-3.5. 닿은 위치에서 일정거리 내리기(8,0,8065) --> 10mm +0.2mm (서포트용)
def thcickdown1(ser, exitcode='*'):
    try:
        print(ser.write(b'8,0,7160'))
        list = []
        while True:
            data = ser.read().decode('utf-8')
            print(ser.readline().decode('utf-8'))
...
#함수1-6. moving & 시료SHOT
def resinMS(ser, exitcode='*'):
    try:
        print(ser.write(b'4,4,4'))
        list = []
        while True:
            data = ser.read().decode('utf-8')
            print(ser.readline().decode('utf-8'))
        ...
        print("resin Moving and shot error")
...
#함수1-6. moving & 시료SHOT
def Rhoming(ser, exitcode='*'):
    try:
        print(ser.write(b'0,0,3'))
        list = []
        while True:
            data = ser.read().decode('utf-8')
            print(ser.readline().decode('utf-8'))
...
def picture():
    try:
        pag.doubleClick(x=1825, y=750)
        #pag.moveTo(1825, 750)
        time.sleep(1)
        pag.click(x=1625, y=150)
...
def suppoter(n):
    i=1
    while i<=n:
        thcickdown1(ser16) #서포터 층(0.2mm만큼 더 내리기)
        time.sleep(15)
        bhoming(ser16) #블레이드 호밍
        littleup(ser16) #베드가 날에 닿기
        bgoing(ser16) #블레이딩
